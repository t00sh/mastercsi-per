...
QEMU est capable de découper un binaire en différents ``basic blocs'', chaque fin de bloc étant repérée grâce à une instruction modifiant le compteur de programme (comme par exemple les instructions \lstinline{jmp}, \lstinline{call} ou encore \lstinline{ret} du jeu d'instructions x86).
...
% TODO: je n'ai pas trop compris en quoi ça résolvait le problème (citation du whitepaper)
% The shift-and-XOR-based scrambling in the second line is used to mask the effects of instruction alignment.
L'opération de shift effectuée à la deuxième ligne est utilisée pour masquer les effets de l'alignement des instructions (arrivant en pratique lorsqu'une instruction est la cible d'un branchement, pour optimiser son chargement par le CPU).
Ce sera donc le cas au début de chacun des basic blocs.
...
Ainsi, le mode QEMU exécute seulement deux à cinq fois plus d'instructions que l'utilisation normale d'AFL.
