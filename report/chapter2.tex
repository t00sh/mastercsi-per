...
Nous avons compilé deux versions de Radare2 : une version normale instrumenté avec \lstinline{afl-gcc} et une autre compilé avec ASAN afin de découvrir de nouveaux problèmes. La première version peut se compiler de cette manière :
...
La version compilée avec ASAN, nous pouvons utiliser ces commandes :
...
Il faut savoir que ASAN peut difficilement être utilisé avec AFL étant donné qu'il demande énormément de mémoire.
...
\begin{itemize}
\item radare2 : il s'agit du programme principale du projet Radare2.
  Celui-ci peut prendre en paramètre des scripts exécutant des commandes et un fichier binaire à analyser.

  Nous avons choisi de tester à la fois le parsing des scripts passés à radare2, et à fois le parser des formats de fichiers binaires.
...
Après environ 3 jours, nous avons découvert un certains nombre de crashs trouvés grâce à nos jeux de tests et AFL.
...
Après quelques jours de fuzzing, nous avons obtenus 10 crashs sur le parsing des commandes \lstinline{radare2} et un crash sur le parsing des fichiers binaires (toujours sur l'outil \lstinline{radare2}).
Les fichiers générant ces crashs sont stockés dans notre dossier \path{out/crashes}, les entrées s'exécutant en plus de une seconde (temps de timeout par défaut de AFL) dans le dossier \path{out/hangs} et l'ensemble des mutations dans le dossier \path{out/queue}.

Afin de découvrir d'autres problèmes potentiels, nous avons ré-exécuté notre version de Radare2 compilée avec ASAN sur l'ensemble des fichiers présents dans \path{out/queue}.
